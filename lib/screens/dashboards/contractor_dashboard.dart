...
List<Map<String, dynamic>> get allComplaints =>
    LegacyDashboardAdapter.detailComplaints(
      ComplaintStore.instance.getContractorComplaints(),
    );
...
class _DeskViewState extends State<_DeskView> {
  List<Map<String, dynamic>> get newComplaints =>
      LegacyDashboardAdapter.detailComplaints(
        ComplaintStore.instance.getContractorComplaints().where((c) {
  ...
  List<Map<String, dynamic>> get assignedComplaints =>
      LegacyDashboardAdapter.detailComplaints(
        ComplaintStore.instance.getContractorComplaints().where((c) {
          final status = (c['status'] ?? '').toString().trim().toLowerCase();
          return status == 'in progress' || status == 'pending approval';
        }),
      );
...
class _ProfileViewState extends State<_ProfileView> {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // 2️⃣ COMPANY INFO SECTION
          _sectionCard(
            title: 'Company Information',
            icon: Icons.business_outlined,
            children: [
              _infoRow('Company', 'Sharma Contractors Pvt Ltd'),
              const Divider(height: 16),
              _infoRow('Contractor ID', 'CNT-SMC-2024-045'),
              const Divider(height: 16),
              _infoRow('Registration', 'GST: 27AABCS1234A1Z5'),
            ],
          ),
          const SizedBox(height: 16),

          // 3️⃣ ACTIVE CONTRACTS SECTION
          _sectionCard(
            title: 'Active Contracts',
            icon: Icons.assignment_outlined,
            children: [
              const Text(
                'Current road maintenance contracts:',
                style: TextStyle(fontSize: 13, color: textSecondary),
              ),
              const SizedBox(height: 12),
              _buildContractChip('Road Resurfacing - Zone A'),
              const SizedBox(height: 8),
              _buildContractChip('Pothole Repair - Ward 12-15'),
              const SizedBox(height: 8),
              _buildContractChip('Emergency Response - City Wide'),
            ],
          ),
          const SizedBox(height: 16),
  ...
  Widget _buildContractChip(String contract) {
    return Container(
