...
class ChiefEngineerDashboard extends StatefulWidget {
  const ChiefEngineerDashboard({super.key});
...
// City Engineer Config
class _CEConfig {
  static const String department = 'Engineering Department';
  static const String name = 'Dr. Anand Sharma';
  static const String employeeId = 'CE-SMC-2024-015';
  static List<String> get zones => WardAssignmentService.assignedWards;
}
...
class _DeskViewState extends State<_DeskView> {
  List<Map<String, dynamic>> get escalatedComplaints =>
      LegacyDashboardAdapter.ceEscalatedComplaints();

  List<Map<String, dynamic>> get highSeverityComplaints =>
      LegacyDashboardAdapter.ceHighSeverityComplaints();

  List<Map<String, dynamic>> get longPendingComplaints =>
      LegacyDashboardAdapter.ceLongPendingComplaints();
  ...
  Widget build(BuildContext context) {
    return ListView(
      ...
      children: [
        ...
        Row(
          children: [
            Icon(Icons.engineering_outlined, color: primary, size: 24),
            const SizedBox(width: 8),
            const Expanded(
              child: Text(
                'City Engineer Desk',
        ...
        Text(
          'Escalated cases & high-priority interventions',
          style: TextStyle(fontSize: 13, color: textSecondary),
        ),
        const SizedBox(height: 20),

        // SECTION A: ESCALATED FROM JE
        _buildSectionHeader(
          'Escalated from Junior Engineers',
          Icons.arrow_upward_rounded,
          Colors.orange,
          '${escalatedComplaints.length} cases',
        ),
        const SizedBox(height: 12),
        if (escalatedComplaints.isEmpty)
          _buildEmptyState('No escalations', 'All JE complaints within SLA.')
        else
        ...
        // SECTION B: HIGH SEVERITY
        _buildSectionHeader(
          'High Severity Issues',
          Icons.warning_amber_rounded,
          Colors.red,
          '${highSeverityComplaints.length} critical',
        ),
        const SizedBox(height: 12),
        if (highSeverityComplaints.isEmpty)
          _buildEmptyState('No high severity', 'No critical issues pending.')
        else
          ...highSeverityComplaints.map(
            (c) => _CEDeskCard(
              data: c,
              onLocationClick: widget.onLocationClick,
              cardType: 'highSeverity',
            ),
          ),
        const SizedBox(height: 24),

        // SECTION C: LONG PENDING
        _buildSectionHeader(
          'Long Pending - Proactive Intervention',
          Icons.schedule_outlined,
          const Color(0xFF9C27B0),
          '${longPendingComplaints.length} cases',
        ),
        const SizedBox(height: 12),
        if (longPendingComplaints.isEmpty)
          _buildEmptyState(
            'No long pending',
            'All complaints progressing well.',
          )
        else
          ...longPendingComplaints.map(
            (c) => _CEDeskCard(
              data: c,
              onLocationClick: widget.onLocationClick,
              cardType: 'longPending',
            ),
          ),
        const SizedBox(height: 20),
      ],
...
// CHIEF ENGINEER DESK CARD
class _CEDeskCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final daysRemaining = isEscalated ? _SLAConfig.getDaysRemaining(data) : 999;
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
          color: isEscalated ? Colors.red.withOpacity(0.03) : Colors.white,
          borderRadius: BorderRadius.circular(12),
          border: isEscalated
        ...
        child: Column(
          ...
          children: [
            ...
            // ESCALATION INFO (for escalated complaints)
            if (isEscalated && data['escalatedFrom'] != null) ...[
              Container(
                padding: const EdgeInsets.symmetric(
                  horizontal: 10,
                  vertical: 6,
            ...
            // ASSIGNED INFO (for high severity complaints)
            if (!isEscalated && data['assignedTo'] != null) ...[
              Container(
                padding: const EdgeInsets.symmetric(
                  horizontal: 10,
                  vertical: 6,
...
class _JEComplaintDetailScreenState extends State<_JEComplaintDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final isVerified =
        ...
        widget.data['status'] == 'In Progress';
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(ctx);
              if ((widget.data['status'] ?? '') == 'Pending CE Authorization') {
                await ComplaintStore.instance.authorizeComplaint(
                  widget.data['id'] as String,
                );
              } else {
                await ComplaintStore.instance.verifyComplaint(
                  widget.data['id'] as String,
                );
              }
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: _selectedContractor == null
                ? null
                : () async {
                    Navigator.pop(ctx);
                    await ComplaintStore.instance.assignComplaint(
                      complaintId: widget.data['id'] as String,
                      assignee: _selectedContractor!,
                      assigneeType: 'Contractor',
                    );
                    if (!context.mounted) return;
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Assigned to $_selectedContractor'),
                      ),
                    );
                  },
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: _selectedWorkGang == null
                ? null
                : () async {
                    Navigator.pop(ctx);
                    await ComplaintStore.instance.assignComplaint(
                      complaintId: widget.data['id'] as String,
                      assignee: _selectedWorkGang!,
                      assigneeType: 'Work Gang',
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(
            hintText: 'Enter escalation remarks',
            border: OutlineInputBorder(),
          ),
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              Navigator.pop(ctx);
              await ComplaintStore.instance.escalateComplaint(
                widget.data['id'] as String,
                remarks: remarks,
              );
              if (!context.mounted) return;
...
class _MapViewState extends State<_MapView> {
  ...
  List<Map<String, dynamic>> get _complaintLocations =>
      LegacyDashboardAdapter.mapLocations([
        ...LegacyDashboardAdapter.ceEscalatedComplaints(),
        ...LegacyDashboardAdapter.ceHighSeverityComplaints(),
        ...LegacyDashboardAdapter.ceLongPendingComplaints(),
      ]);
      ...
      child: Column(
        ...
        children: [
          ...
          Center(
            child: Column(
              children: [
                ...
                const SizedBox(height: 16),
                Text( _userProfile?['name']?.toString() ?? 'Dr. Anand Sharma', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: textPrimary) ),
                const SizedBox(height: 4),
          ...
          _sectionCard(
            ...
            children: [
              _infoRow('Department', _CEConfig.department),
              const Divider(height: 16),
              _infoRow('Designation', 'City Engineer'),
              const Divider(height: 16),
              _infoRow('Employee ID', _CEConfig.employeeId),
            ],
          ),
          const SizedBox(height: 16),

          // 3️⃣ ZONES UNDER CONTROL SECTION
          _sectionCard(
            title: 'Zones Under Control',
            icon: Icons.location_on_outlined,
            children: [
              const Text(
                'Supervisory responsibility for these zones:',
                style: TextStyle(fontSize: 13, color: textSecondary),
              ),
              const SizedBox(height: 12),
              Wrap(
