...
class AssistantEngineerDashboard extends StatefulWidget {
  const AssistantEngineerDashboard({super.key});

  @override
  State<AssistantEngineerDashboard> createState() =>
      _AssistantEngineerDashboardState();
}

class _AssistantEngineerDashboardState
    extends State<AssistantEngineerDashboard> {
  int _selectedIndex = 0;
  final GlobalKey<_MapViewState> _mapKey = GlobalKey<_MapViewState>();

  final List<String> _titles = [
    'Assistant Engineer Home',
    'My Desk',
    'Complaint Map',
    'City Activity',
    'My Profile',
...
class _SLAConfig {
  ...
  static bool isNearEscalation(Map<String, dynamic> complaint) {
    return getDaysRemaining(complaint) <= 1;
  }
...
// Assistant Engineer Assigned Wards (Mock data)
class _AEConfig {
  static const String zone = 'North Zone';
  static const List<String> subZones = [
    'North Central',
    'North East',
    'North West',
  ];
  static List<String> get assignedWards => WardAssignmentService.assignedWards;
  static const int jesUnderSupervision = 3;
  static const String department = 'Engineering Department - Zonal';
  static const String name = 'Rajesh Kumar';
  static const String employeeId = 'AE-Z1-001';
}
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        Row(
          children: [
            ...
            Container(
              ...
              child: Text(
                '${_AEConfig.assignedWards.length} Wards',
                style: const TextStyle(
                  fontSize: 12,
        ...
        Text(
          'Zone-level oversight and escalated complaints',
          style: TextStyle(fontSize: 13, color: textSecondary),
        ),
        const SizedBox(height: 20),

        // SECTION 1: ESCALATED FROM JE (Priority)
        if (escalationRiskComplaints.isNotEmpty) ...[
          _buildSectionHeader(
            'Escalated from Junior Engineers',
            Icons.arrow_upward_rounded,
            Colors.red,
            '${escalationRiskComplaints.length} breached JE SLA',
          ),
          const SizedBox(height: 12),
        ...
        // SECTION 2: ALL ZONE COMPLAINTS (Read-Only Visibility)
        _buildSectionHeader(
          'All Zone Complaints',
          Icons.visibility_rounded,
          const Color(0xFF4A90D9),
          '${newComplaints.length} in progress',
        ),
        const SizedBox(height: 12),
        if (newComplaints.isEmpty)
          _buildEmptyState(
            'No active complaints',
            'All complaints in your zone are resolved.',
          )
        else
          ...newComplaints.map(
            (c) => _AEDeskCard(
        ...
        // SECTION 3: NEAR SLA BREACH (6-day limit approaching)
        _buildSectionHeader(
          'Near SLA Breach',
          Icons.timer_outlined,
          Colors.orange,
          '${assignedComplaints.length} approaching limit',
        ),
        const SizedBox(height: 12),
        if (assignedComplaints.isEmpty)
          _buildEmptyState(
            'No approaching breach',
            'All complaints are within SLA limits.',
          )
        else
...
// Assistant Engineer DESK CARD
class _AEDeskCard extends StatelessWidget {
  final Map<String, dynamic> data;
  final Function(LatLng, String) onLocationClick;
  final String cardType; // 'escalated', 'zone', 'sla'
  ...
  Widget build(BuildContext context) {
    ...
    final showSLAIndicator = cardType != 'zone';
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // SLA INDICATOR (for escalated and sla complaints)
            if (showSLAIndicator) ...[
              Container(
                padding: const EdgeInsets.symmetric(
                  horizontal: 10,
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    const SizedBox(width: 4),
                    Text(
                      isEscalated ? 'escalated to AE' : 'before DE escalation',
                      style: TextStyle(
                        fontSize: 11,
            ...
            // ASSIGNED INFO (for zone view complaints)
            if (!showSLAIndicator && data['assignedTo'] != null) ...[
              Container(
                padding: const EdgeInsets.symmetric(
                  horizontal: 10,
                  vertical: 6,
...
// Assistant Engineer COMPLAINT DETAIL SCREEN
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          // ACTION BUTTONS / VERIFICATION FRAMEWORK
          if (canOpenVerificationFramework && isNew) ...[
            _buildSectionTitle('Verification Framework'),
            const SizedBox(height: 12),
            _buildActionButton(
              'Verify',
        ...
        builder: (_) => VerifyComplaintScreen(
          ...
          onVerified: () {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text(
                  'Complaint submitted for City Engineer authorization',
                ),
              ),
            );
          },
      ...
      child: Column(
        ...
        children: [
          ...
          _sectionCard(
            ...
            children: [
              _infoRow('Department', _AEConfig.department),
              const Divider(height: 16),
              _infoRow('Designation', 'Assistant Engineer (Roads)'),
              const Divider(height: 16),
              _infoRow('Employee ID', _AEConfig.employeeId),
            ],
          ),
          const SizedBox(height: 16),

          // 3️⃣ ASSIGNED WARDS SECTION
          _sectionCard(
            title: 'Assigned Wards',
            icon: Icons.location_on_outlined,
            children: [
              const Text(
                'You are responsible for complaints in these wards:',
                style: TextStyle(fontSize: 13, color: textSecondary),
              ),
              const SizedBox(height: 12),
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: _AEConfig.assignedWards
                    .map(
                      (ward) => Container(
                        padding: const EdgeInsets.symmetric(
                          horizontal: 12,
